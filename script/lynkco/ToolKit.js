  ...
  class Request {
    ...
    fetch(options, method = "GET") {
      ...
      const doFetch = new Promise((resolve, reject) => {
        fetcher.call(this, options, (error, resp, data) =>
          error
            ? reject({
                error,
                resp,
      ...
      return options.timeout > 0
        ? delayFetch(doFetch, options.timeout)
        : doFetch;
    }
  ...
  return new (class {
    ...
    async execComm() {
      ...
      if (this.isEmpty(this?.options?.httpApi)) {
        ...
        if (typeof httpApi == "object") {
          targetDevice = this.isNumeric(this.comm[2])
            ? this.comm[3] || "unknown"
            : this.comm[2];
    ...
    callApi(timeout, targetDevice, httpApi) {
      ...
      this.req.post(options).then(({ error, resp, data }) => {
        this.log(
          `已将脚本【${fname}】发给【${targetDevice}】,执行结果: \n${
            this.twoSpace
          }error: ${error}\n${this.twoSpace}resp: ${resp?.s()}\n${
            this.twoSpace
          }data: ${this.responseDataAdapter(data)}`
        );
        this.done();
      });
    }
    ...
    boxJsJsonBuilder(info, param) {
      ...
      let boxjsJsonPath =
        param?.targetBoxjsJsonPath ||
        "/Users/lowking/Desktop/Scripts/lowking.boxjs.json";
      ...
      boxJsJson.icons = [
        `${domain}/mini/master/Alpha/${this.id.toLocaleLowerCase()}.png`,
        `${domain}/mini/master/Color/${this.id.toLocaleLowerCase()}.png`,
      ];
      boxJsJson.keys = [];
      boxJsJson.settings = [
        {
      ...
      const {
        ...
        isCurDirDataFile,
        isRootDirDataFile,
      } = this.checkPath(this.boxJsJsonFile);
    ...
    msg(subtitle, message, openUrl, mediaUrl, copyText, disappearS) {
      ...
      if (this.isEnableTgNotify) {
        ...
        const isMarkdown =
          this.tgNotifyUrl &&
          this.tgNotifyUrl.indexOf("parse_mode=Markdown") != -1;
        if (isMarkdown) {
    ...
    costTime() {
      ...
      info = `${info}\n${this.twoSpace}耗时【${costTime}】秒(含休眠${
        this.sleepTotalMs ? (this.sleepTotalMs / 1000).toFixed(4) : 0
      }秒)`;
    ...
    isMatch(reg) {
      return !!(
        $request.method != "OPTIONS" && this.getRequestUrl().match(reg)
      );
    }
